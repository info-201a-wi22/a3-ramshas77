...
display_chart1 <- function(data) {
  ...
  chart1 <- ggplot(trend_over_time) +
    geom_col(aes(x = year, y = nationwide_black_jail_population)) + 
...
display_map <- function(data) {
  black_incarcerated_proportion <- data %>% filter(year == 2016) %>% select(year, state, black_pop_15to64, black_prison_pop) %>%
    na.omit() %>%
    group_by(state) %>% summarize(total_black_population = sum(black_pop_15to64),
                                  total_black_incarcerated_population = sum(black_prison_pop)) %>%
    mutate(black_incarcerated_proportion = total_black_incarcerated_population/total_black_population)
  map <- plot_usmap(data = black_incarcerated_proportion, 
             values = "black_incarcerated_proportion", 
             color = "red") +
    scale_fill_continuous(low = "white", high = "red", name = "ratio") +
    labs(title = "Ratio of the incarcerated black population over the entire black population",
         subtitle = "as of 2016")
  return(map)
}
